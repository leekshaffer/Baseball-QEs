...
ests_tbl <- function(display_name,statval) {
  ...
  if (statval=="-All-") {
    Tbl %>% dplyr::arrange(Season,Outcome)
  } else {
    Tbl %>% dplyr::filter(Outcome==statval) %>% dplyr::arrange(Season)
  }
}
...
plot_diff <- function(display_name,statval) {
  ggplot(data=SCs_Results %>% 
    ...
    scale_linetype_manual(name="",
                       breaks=c(FALSE,TRUE),
                       labels=c(paste0("Target Player: ",display_name),"Placebo Players (2022 Shift Rate \U2264 20%)"),
                       values=c("solid","longdash")) +
    geom_vline(xintercept=Interv-0.5,
               color="grey50", linetype="dashed") +
    theme_bw() + theme(legend.position="bottom") +
    labs(title=paste0("SCM estimates for ",statval," for ",display_name," and placebos"))
}
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("InName", label = h3("Choose Player"), 
                  choices = as.list(Player_Choices), 
                  selected = "Corey Seager"),
      selectInput("InStat", label = h3("Choose Outcome"),
                  choices = as.list(c("-All-",BStats_Use$stat))),
...
server <- function(input, output) {
  ...
  output$tbl2 <- DT::renderDataTable(
    DT::datatable(wts_tbl(input$InName, input$InStat), 
                  options = list(lengthMenu = list(c(2, 5, 10, -1), c('2', '5', '10', 'All')),
                                 pageLength = 5))
  )
